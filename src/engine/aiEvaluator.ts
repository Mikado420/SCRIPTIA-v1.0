...
export class AIEvaluator {
  ...
  public evaluateCandidateAction(
    ...
    let barrierAdvantage = (me.barrier / 5.0) * 5.0 + ((5 - opp.barrier) / 5.0) * 5.0;
    ...
    switch (act.type) {
      ...
      case 'ATTACK': {
        ...
        const stats = attacker ? this.engine.calculateEffectiveStats(attacker, me, opp) : { atk: 0, def: 0, brk: 0 };

        if (payload.targetType === 'PLAYER') {
          pressure = 8.5;
          if (opp.barrier <= stats.brk) {
          ...
          const defStats = defender ? this.engine.calculateEffectiveStats(defender, opp, me) : { atk: 0, def: 0, brk: 0 };
      ...
      case 'GUARD': {
        ...
        if (payload.doGuard) {
          barrierAdvantage = 8.0;
          risk = 4.0;
          rationale = `ガードによりプレイヤー結界への被ダメージを防御。`;
        } else {
          barrierAdvantage = 4.0;
          risk = 5.0;
    ...
    return {
      ...
      breakdown: {
        ...
        barrierAdvantage: parseFloat(barrierAdvantage.toFixed(1)),
