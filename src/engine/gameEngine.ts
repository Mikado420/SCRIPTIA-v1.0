...
export class GameEngine {
  ...
  public calculateEffectiveStats(
    ...
    owner: PlayerState,
    opp: PlayerState
  ): { atk: number; def: number; brk: number; hasGuard: boolean; canAttack: boolean } {
    ...
    for (const buff of unit.buffs) {
      ...
      if (buff.type === 'BRK') brk += buff.value;
          ...
          if (attacker) {
            if (doGuard && guardInstanceId) {
              ...
              if (guardUnit) {
                ...
                logType = 'COMBAT';
                guardUnit.isRested = true; // Rule 19
  ...
  private resolveDirectDamage(
    ...
    defenderPlayer.barrier = Math.max(0, defenderPlayer.barrier - stats.brk);
    attackerPlayer.totalBarrierBroken += stats.brk;
    this.addLog(
      state,
      'DAMAGE',
      `「${attacker.baseCard.name}」の攻撃が成功！ ${defenderPlayer.name} の結界が ${stats.brk} 減少！(残り結界: ${defenderPlayer.barrier})`,
  ...
  private checkGameOutcome(state: GameState): void {
    ...
    if (state.turnNumber >= 100 && state.winner === null) {
      state.winner = pA.barrier > pB.barrier ? 'PLAYER_A' : pB.barrier > pA.barrier ? 'PLAYER_B' : 'DRAW';
      state.winReason = `最大ターン(100)到達判定 (${pA.name} 結界:${pA.barrier} vs ${pB.name} 結界:${pB.barrier})`;
      state.gameStatus = 'FINISHED';
    }
  }
}
