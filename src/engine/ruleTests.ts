...
export function runAllRuleTests(): TestResult[] {
  ...
  // 2. Initial State Rules Test (Rules Ver.0.03: Kekkai 5, Hand 4, Deck 36)
  {
    ...
    if (
      state.playerA.barrier !== 5 ||
      state.playerB.barrier !== 5 ||
      ...
      passed = false;
      message = `初期ゲーム状態の値が不正です (A結界:${state.playerA.barrier}, B結界:${state.playerB.barrier}, A手札:${state.playerA.hand.length}, Aデッキ:${state.playerA.deck.length})`;
    }
  ...
  // 4. Barrier Damage (BRK Direct Attack)
  {
    ...
    const nextState = stepResult.nextState;
    const passed = nextState.playerB.barrier === 3; // 5 - 2 = 3
    ...
    results.push({
      ...
      passed,
      message: passed
        ? `BRK 2 による直接攻撃で相手の結界が 5 → ${nextState.playerB.barrier} に減少しました。`
        : `結界ダメージ計算が不正です (現在結界: ${nextState.playerB.barrier})`,
      durationMs: parseFloat((performance.now() - start).toFixed(2)),
    });
  }
