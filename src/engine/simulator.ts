...
export class MatchSimulator {
  ...
  public playSingleMatch(
    ...
    totalTurns: number;
    finalBarrierA: number;
    finalBarrierB: number;
    replay?: GameReplay;
    ...
    return {
      ...
      totalTurns: state.turnNumber,
      finalBarrierA: state.playerA.barrier,
      finalBarrierB: state.playerB.barrier,
      replay,
      cardUsedInGameA,
      cardUsedInGameB,
      cardKillsA,
  ...
  public async runBatchSimulation(options: SimulationOptions): Promise<{
    ...
    while (matchIdx < totalMatches) {
      ...
      for (; matchIdx < batchEnd; matchIdx++) {
        ...
        const targetFinalHp = isTargetPlayerFirst ? matchResult.finalBarrierA : matchResult.finalBarrierB;
        ...
        if (mStat) {
          ...
          mStat.avgFinalBarrier += targetFinalHp;
        }
    ...
    const matchups: MatchupStat[] = Array.from(matchupMap.values()).map((m) => ({
      ...
      avgFinalBarrier: m.totalMatches > 0 ? parseFloat((m.avgFinalBarrier / m.totalMatches).toFixed(1)) : 0,
    }));
    ...
    const report: VerificationReport = {
      ...
      avgFinalBarrier: totalMatches > 0 ? parseFloat((totalFinalHpAccum / totalMatches).toFixed(1)) : 0,
      maxWinStreak,
      maxLossStreak,
      matchups,
      cardStats,
      ...
      aiDecisionInsights: {
        ...
        commonStrategies: [
          ...
          '終盤(7+ターン): 進化ユニット・ドメインのシナジーを活用し、相手結界を一気にブレイク圏内へ',
        ],
      },
    };
