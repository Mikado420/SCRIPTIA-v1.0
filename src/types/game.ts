...
export interface CardBuff {
  id: string;
  type: 'ATK' | 'DEF' | 'BRK' | 'GUARD' | 'IGNORE_GUARD' | 'CAN_ATTACK' | 'CANT_BE_GUARDED';
  value: number;
...
export interface PlayerState {
  playerId: PlayerId;
  name: string;
  barrier: number; // 結界 (Barrier value, starts at 5)
  maxBarrier: number; // 最大結界 (5)
  deck: CardInstance[];
  hand: CardInstance[];
  arcana: ArcanaSlot[];
  ...
  totalBarrierBroken: number; // Total barrier broken
